 ...
 * the boundary: the standard stat formula's effort term is floor(ev/4), and
 * each Champions SP is worth *2* of that pre-divided term (not 1) - so
 * SP=32 (max) resolves to an effort of 64, not the traditional EV scale's
 * own cap of 63 (=floor(252/4)). Getting this wrong (treating 1 SP as 1
 * effort point, i.e. ev=sp*4) understates every SP's real impact by half;
 * confirmed against live examples: 0/32 SP Modest Mega Lucario Special
 * Attack is 202/237, and 0/32 SP Jolly Garchomp Attack (nature-neutral) is
 * 150/182 - both only come out right with ev=sp*8 (so floor(ev/4)=sp*2).
