...
export default function LikelySetsPopover({ usage, hideAbility, hideItem, confirmedMoves, onClose }: LikelySetsPopoverProps) {
  ...
  return (
    <div ref={ref} className="absolute z-20 top-full mt-1 left-0 w-56 p-2 rounded-lg bg-zinc-800 border-2 border-dashed border-amber-700 shadow-lg flex flex-col gap-1.5">
      ...
      {abilities.length > 0 && (
        <div className="flex flex-col gap-0.5">
          <span className="text-[9px] text-zinc-500 uppercase">Ability</span>
          {abilities.map(a => (
            <div key={a.name} className="flex items-center justify-between text-[10px] text-zinc-200">
              <span className="truncate">{a.name}</span>
              <span className="text-zinc-500 shrink-0 ml-1">{a.percentage.toFixed(1)}%</span>
            </div>
          ))}
        </div>
      )}
      {items.length > 0 && (
        <div className="flex flex-col gap-0.5">
          <span className="text-[9px] text-zinc-500 uppercase">Item</span>
          {items.map(i => (
            <div key={i.name} className="flex items-center justify-between text-[10px] text-zinc-200">
              <span className="truncate">{i.name}</span>
              <span className="text-zinc-500 shrink-0 ml-1">{i.percentage.toFixed(1)}%</span>
            </div>
          ))}
        </div>
      )}
      {moves.length > 0 && (
        <div className="flex flex-col gap-0.5">
          <span className="text-[9px] text-zinc-500 uppercase">Moves</span>
          {moves.map(m => (
            <div key={m.name} className="flex items-center justify-between text-[10px] text-zinc-200">
              <span className="truncate">{m.name}</span>
              <span className="text-zinc-500 shrink-0 ml-1">{m.percentage.toFixed(1)}%</span>
            </div>
          ))}
        </div>
      )}
      {natures.length > 0 && (
        <div className="flex flex-col gap-0.5">
          <span className="text-[9px] text-zinc-500 uppercase">Nature</span>
          {natures.map(n => (
            <div key={n.name} className="flex items-center justify-between text-[10px] text-zinc-200">
              <span className="truncate">{formatNature(n.name, n.statUp, n.statDown)}</span>
              <span className="text-zinc-500 shrink-0 ml-1">{n.percentage.toFixed(1)}%</span>
            </div>
          ))}
        </div>
      )}
      {statSpreads.length > 0 && (
        <div className="flex flex-col gap-0.5">
          <span className="text-[9px] text-zinc-500 uppercase">Stat Points</span>
          {statSpreads.map((s, i) => (
            <div key={i} className="flex items-center justify-between text-[10px] text-zinc-200 gap-1">
              <span className="truncate">{formatStatSpread(s.points)}</span>
              <span className="text-zinc-500 shrink-0">{s.percentage.toFixed(1)}%</span>
            </div>
          ))}
        </div>
      )}
      <span className="text-[8px] text-zinc-600">Champions ranked usage · {usage.season} · updated {formatCacheAge(usage.cachedAt)}</span>
    </div>
  );
}
