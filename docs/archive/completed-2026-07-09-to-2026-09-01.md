# ChoiceBuds - Completed Work Log Archive (2026-07-09 - 2026-09-01)

Rolled out of [COMPLETED.md](../../COMPLETED.md) on 2026-09-08, at the Card
UI Polish milestone boundary. This is the second split and the first done
under the milestone-boundary convention rather than the rolling ~50-entry
window the first split used - per CLAUDE.md's archiving rules, the first
milestone reached becomes the real cutoff and the rolling-window habit
stops. Contains every entry from the Battle Logger Re-eval + Data & Process
Cleanup milestone and earlier (everything up through File Size Cap Cleanup,
that milestone's own closing item) that hadn't already been archived by the
first split. Newest-first within this file too, same as the live log.
Cross-references to still-open items point to `TODO.md`; references to other
entries stay local to whichever file they were written in ("see
below"/"see above") - none of this split's internal references cross the
split boundary.
