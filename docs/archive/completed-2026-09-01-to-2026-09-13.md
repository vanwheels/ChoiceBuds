# ChoiceBuds - Completed Work Log Archive (2026-09-01 - 2026-09-13)

Rolled out of [COMPLETED.md](../../COMPLETED.md) on 2026-09-13, at the
Regular Calc Popup milestone boundary. Contains every entry from Card UI
Polish (the milestone underway when the prior split closed) through Regular
Calc Popup, inclusive - i.e. every milestone shipped in between too:
Maintenance & Investigation Sweep, Live Calc: Damage-Based Stat Inference
Tab, Regulation M-C Prep, Speed Calc-like Feature, UI Polish & Performance,
Battle Logger Overhaul, Statistics Improvements, Team Management QoL, Saved
Builds Box, and Building Flow Tweaks (see `MILESTONES.md` for each one's
post-mortem). Newest-first within this file too, same as the live log.
