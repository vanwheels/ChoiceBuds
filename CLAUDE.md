...
- **Commit messages**: the user commits and pushes through GitHub Desktop themselves rather than asking Claude to run `git commit`/`git push`. Proactively draft a commit summary + description after finishing a meaningful checkpoint (a `TODO.md` item, a live-verified fix) — don't wait to be asked. Put the summary and description in two separate fenced code blocks (not embedded together with prose) so each can be copy-pasted independently. Don't run `git commit`/`git push` unless the user separately asks for that.
- **GitHub Releases** (decided 2026-07-09): the reverse split from commits — Claude handles these via `gh release create`, not the user. Draft the title + body the same two-fenced-code-block way as a commit message, then actually run `gh release create` (tag matching `package.json`'s `version` with a leading `v`, e.g. `v0.1.0` for `"0.1.0"` — the update checker in `services/github.ts` strips the `v` before comparing, so they must match) once the user confirms the content and gives the go-ahead — a Release is a visible, external, public-facing action, so always confirm before publishing one, same as any other action with that risk profile.
